...
## Почему она не встроена в панель задач

Встроить своё окно внутрь панели задач Windows 11 сторонняя программа не может.
Раньше для этого существовали «панели инструментов» (deskbands) — так работал,
например, старый ввод адреса на панели, — но в Windows 10 их объявили устаревшими,
а в Windows 11 панель задач переписали, и чужие элементы в неё больше не
встраиваются. Поиск, виджеты и часы — части самого проводника, а не отдельные
программы.

Поэтому полоска просто висит над панелью задач и всегда поверх окон, а пункт меню
**«Прижать к панели задач»** ставит её в угол. Следующая ступень, если понадобится —
зарегистрировать окно как *панель рабочего стола* (AppBar): Windows тогда отдаст
ему полосу экрана у края и перестанет накрывать его развёрнутыми окнами, как
второй панели задач.
