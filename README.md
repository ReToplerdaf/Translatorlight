...
| Замочек на полоске | Закрепляет её на месте: перетащить уже не выйдет |
...
Полоску таскают за рамку поля и за полосы над и под ним — там, где курсор
становится крестиком со стрелками; ширину меняют, потянув за правый край. Высота
всегда одна строка — она и не должна быть больше. Закрытый замочек запрещает
перетаскивание, чтобы полоска не уехала от случайного движения мышью; ширина при
этом по-прежнему меняется.

На самой полоске только поле, замочек и крестик. Направление перевода, «поверх
всех окон» и прозрачность живут в меню значка — их трогают редко.

Подсказки виджет пишет серым прямо в поле: пока оно пустое — «перетащите текст»,
а пока идёт перевод — «обрабатывается». Слева направо это выглядит так: бросили
текст, слово исчезло, на его месте появилось «обрабатывается», а потом — перевод.
Постоянной строки состояния нет; красная надпись справа появляется только если
перевести не удалось, и подсказка к ней объясняет, что именно случилось.
...
В меню значка направление можно закрепить: **Всегда EN → RU** или
**Всегда RU → EN**. Текущее направление показывает подсказка значка в трее.
...
- **Закрепить на месте** — то же, что замочек на полоске: запрещает перетаскивание.
- **Панель рабочего стола** — приклеить полоску к верхнему или нижнему краю экрана,
  чтобы Windows держала за её границей развёрнутые окна.
- **Сразу копировать перевод** — класть перевод в буфер обмена. По умолчанию включено.
- **Прозрачность, когда не активен** — полоска становится чуть прозрачной, когда
  вы работаете в другом окне, и снова плотной, когда на неё тянут текст. Выключите,
  чтобы она всегда была непрозрачной.
...
## Полосой во весь экран, как вторая панель задач

В меню значка есть **«Панель рабочего стола»** — там полоску можно приклеить к
верхнему или нижнему краю экрана. Тогда она перестаёт быть окном: Windows
отдаёт ей свою полосу экрана и держит развёрнутые окна за её границей, ровно как
для панели задач. Ничто её больше не перекрывает, и целиться в неё не нужно —
она всегда на своём месте.

![Полоска, закреплённая у нижнего края](docs/docked.png)

Встроить окно **внутрь** самой панели задач Windows 11 сторонняя программа не
может. Раньше для этого существовали «панели инструментов» (deskbands) — так
работал, например, старый ввод адреса на панели, — но в Windows 10 их объявили
устаревшими, а в Windows 11 панель задач переписали, и чужие элементы в неё
больше не встраиваются: поиск, виджеты и часы — части самого проводника.
Закреплённая полоса — самое близкое, что доступно обычной программе.

Пока полоска закреплена, её не перетащить и не растянуть: положением и шириной
распоряжается Windows. Замочек в этом режиме пропадает — держать полоску больше
не от чего, — и поле занимает освободившееся место. Ширина и место плавающей полоски при этом запоминаются
отдельно, так что «Не закреплять» вернёт её туда, где она была.

> [!NOTE]
> Полоса экрана остаётся за программой, пока та не вернёт её обратно. Виджет
> отдаёт её, когда вы его прячете, откепляете или закрываете. Если после
> аварийного завершения полоса всё же осталась занятой, перезапуск проводника
> (или выход из системы) освободит её.
...
Если поставить тег вида `v0.2.0`, сборка сама создаст [релиз](../../releases) и
приложит к нему exe — тогда файл можно будет скачать прямо оттуда.
...
| `src/Translatorlight/DesktopBar.cs` | Закрепление полоски у края экрана средствами оболочки |
| `src/Translatorlight/SmoothPanel.cs` | Панели, которые перерисовываются целиком при смене ширины |
...
Серая подсказка в поле — не только `PlaceholderText`, но и надпись, лежащая
поверх пустого поля: так её видно наверняка, в том числе ту, что показывается
во время перевода. Когда в поле появляется курсор, подсказка уходит, чтобы не
закрывать его, — кроме «обрабатывается», которое как раз и нужно видеть.

Обычная панель перерисовывает только ту полосу себя, которая только что
открылась при растягивании. Рамка, нарисованная по её правому краю, от этого
оставалась стоять посреди поля синей чертой, поэтому и поле, и таблица разметки,
и само окно перерисовываются целиком при смене ширины — и делают это в буфере,
чтобы не мигать.

Закрепление у края — это `SHAppBarMessage`: окно регистрируется в оболочке как
панель рабочего стола, спрашивает у неё полосу нужной толщины, получает
согласованный прямоугольник и переезжает в него. Оболочка потом сама сообщает,
когда прямоугольник надо запросить заново — например, если панель задач
переехала или открылась ещё одна такая панель. Полоса числится за программой до
явного возврата, поэтому возврат стоит на всех выходах разом: откепление,
скрытие, уничтожение окна и завершение процесса. Ещё оболочка предупреждает,
когда открывается полноэкранная программа, — на это время полоска перестаёт быть
поверх всех окон, чтобы не лезть в кадр.
