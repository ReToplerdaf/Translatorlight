...
Полоску таскают за точки слева (и вообще за любое пустое место), ширину меняют,
потянув за правый край. Высота всегда одна строка — она и не должна быть больше.
Закрытый замочек запрещает перетаскивание, чтобы полоска не уехала от случайного
движения мышью; ширина при этом по-прежнему меняется.

На самой полоске только то, что нужно каждый день: поле, короткий статус, замочек
и крестик. Направление перевода и «поверх всех окон» живут в меню значка — их
трогают редко.
...
распоряжается Windows. Ширина и место плавающей полоски при этом запоминаются
